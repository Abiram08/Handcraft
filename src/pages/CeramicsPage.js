    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/products', {
          params: { category: 'ceramics' }, // Removed status: 'approved'
        });
        console.log('Fetched ceramics:', response.data);
        setProducts(Array.isArray(response.data) ? response.data : []);
        setError('');
      } catch (err) {
        console.error('Error fetching ceramics:', err);
      ...
      {loading ? (
        ...
        <div className="product-grid">
          {products.length > 0 ? (
            ...
            <p>No ceramics found.</p>
          )}
        </div>
      )}

      {/* Popup for Full Product Details */}
      {selectedProduct && (
          ...
          <div className="popup-content">
            ...
            <div className="popup-details">
              ...
              <div className="popup-text">
                ...
                <p><strong>Stock:</strong> {selectedProduct.countInStock ?? 'N/A'}</p>
